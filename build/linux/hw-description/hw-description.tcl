...
proc is_ip_interrupt_connected {srcname} {
	...
	foreach p ${intr_pins} {
		...
		if { [catch {set intr_id [hsm::utils::get_interrupt_id "${srcname}" "${intr_port_name}"]} errmsg] } {
			# This is the workaround to interrupt detecting errors
			# when interrupt is routed external but not to interrupt controller
			return 1
		}
		if { [string match -nocase $intr_id "-1"] } {
			set intr_id [hsi::utils::get_port_intr_id "$srcname" "$intr_port_name" ]
		}
		if { [string match -nocase $intr_id "-1"] } {
			return -1
		} else {
